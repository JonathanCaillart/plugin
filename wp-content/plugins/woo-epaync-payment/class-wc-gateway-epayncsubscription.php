...
class WC_Gateway_EpayncSubscription extends WC_Gateway_EpayncStd
{
    ...
    {
        $this->id = 'epayncsubscription';
        $this->icon = apply_filters('woocommerce_epayncsubscription_icon', WC_EPAYNC_PLUGIN_URL . 'assets/images/epaync.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Subscription payment', 'woo-epaync-payment');
        ...
        $this->subscriptions_handler = Epaync_Subscriptions_Loader::getInstance($handler);
        ...
        add_filter('woocommerce_order_needs_payment', array($this, 'epaync_order_needs_payment'), 10, 2);

        add_action('manage_shop_subscription_posts_custom_column', array($this, 'epaync_display_subscription_error_msg'), 2);
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables payment by Subscription.', 'woo-epaync-payment');

        // Add subscription payment fields.
        $this->form_fields['subscriptions'] = array(
            'title' => __('Subscriptions management', 'woo-epaync-payment'),
            'type' => 'select',
            'default' => 'wc-subscriptions',
            'options' => array(
                'wc-subscriptions' => 'WooCommerce Subscriptions',
                'subscriptio' => 'Subscriptio 2.x',
                'custom' => __('Custom', 'woo-epaync-payment')
            ),
            'description' => __('If you buy subscriptions on your site, choose the solution you use to manage them. If you choose "Custom", your developper may develop a subscriptions adapter for our plugin.', 'woo-epaync-payment'),
    ...
    public function payment_fields()
    {
        ...
        if ($this->subscriptions_handler && $this->subscriptions_handler->is_subscription_update()) {
            ...
            echo '<input type="hidden" id="epayncsubscription_old_pm" name="epayncsubscription_old_pm" value="' . $method . '">';
        }
    }
    ...
    protected function payment_by_alias_view($html, $force_redir = true)
    {
        ...
        echo '<div id="' . $this->id . '_payment_by_token_description">
                  <ul>
                      <li>
                          <span>' .
                              sprintf(__('You will pay with your stored means of payment %s', 'woo-epaync-payment'), $saved_subsc_masked_pan)
    ...
    protected function epaync_fill_request($order)
    {
        ...
        if (is_array($info) && ! empty($info)) {
            ...
            $this->epaync_request->set('sub_amount', $currency->convertAmountToInteger($info['amount']));
            $this->epaync_request->set('sub_currency', $currency->getNum()); // Same as general order currency.

            $this->epaync_request->set('sub_desc', $this->get_rrule($info));

            $this->epaync_request->set('sub_effect_date', $info['effect_date']);

            // Initial amount.
            if (isset($info['init_amount']) && $info['init_amount'] && isset($info['init_number']) && $info['init_number']) {
                $this->epaync_request->set('sub_init_amount', $currency->convertAmountToInteger($info['init_amount']));
                $this->epaync_request->set('sub_init_amount_number', $info['init_number']);
            }

            $order_amount = $order->get_total();
            if ($order_amount > 0) {
                $this->epaync_request->set('page_action', 'REGISTER_PAY_SUBSCRIBE');
                $this->epaync_request->set('identifier', null);
            } else {
                // Only subscriptions.
                if ($saved_identifier && $is_identifier_active) {
                    $this->epaync_request->set('identifier', $saved_identifier);
                    $this->epaync_request->set('page_action', 'SUBSCRIBE');
                } else {
                    $this->epaync_request->set('page_action', 'REGISTER_SUBSCRIBE');
                }
            }
        } elseif ($saved_identifier) {
            // Called from change payment action.
            $this->epaync_request->set('amount', 0);
            $this->epaync_request->set('identifier', $saved_identifier);
            $this->epaync_request->set('page_action', 'REGISTER_UPDATE');
        }
        ...
        if ($parent_order = $this->subscriptions_handler->get_parent_order($order_id)) {
            $this->epaync_request->set('order_id', self::get_order_property($parent_order, 'id'));
            $this->epaync_request->addExtInfo('order_key', self::get_order_property($parent_order, 'order_key'));
            $this->epaync_request->addExtInfo('subsc_id', $order_id);
        }
    }
    ...
    public function cancel_online_subscription($subscription_id, $order_id)
    {
        ...
        if (! $key) {
            if (is_admin()) { // Show error message only if it's made on backend.
                set_transient('epaync_cancelled_subscription_error_msg', sprintf(__('Subscription is cancelled only in WooCommerce. Please, consider cancelling the subscription in %s Back Office.', 'woo-epaync-payment'), 'EpayNC'));
            } else {
                wc_add_notice( __('An error occurred during the cancellation of the subscription. Please contact customer support.', 'woo-epaync-payment'), 'notice' );
            }
        ...
        try {
            ...
            if (is_admin()) { // Show error message only if it's made on backend.
                set_transient('epaync_cancelled_subscription_error_msg', sprintf(__('An error has occurred during the cancellation or update of the subscription. Please consult the %s logs for more details.', 'woo-epaync-payment'), 'EpayNC'));
            } else {
                wc_add_notice( __('An error occurred during the cancellation of the subscription. Please contact customer support.', 'woo-epaync-payment'), 'notice' );
            }
    ...
    private function cancel_transaction($transaction)
    {
        ...
        try {
            ...
            EpayncRestTools::checkResult($result, 'CANCELLED');
        } catch (Exception $e) {
            if (is_admin()) { // Show error message only if it's made on backend.
                set_transient('epaync_cancelled_subscription_error_msg', sprintf(__('An error has occurred during the cancellation or update of the subscription. Please consult the %s logs for more details.', 'woo-epaync-payment'), 'EpayNC'));
            }  else {
    ...
    public function update_online_subscription($subscription_id, $order_id)
    {
        $key = $this->testmode ? $this->get_general_option('test_private_key') : $this->get_general_option('prod_private_key');
        if (! $key) {
            set_transient('epaync_update_subscription_error_msg', sprintf(__('Subscription is updated only in WooCommerce. Please, consider making necessary changes in %s Back Office.', 'woo-epaync-payment'), 'EpayNC'));
        ...
        $info = $this->subscriptions_handler->subscription_info($order);
        if (! is_array($info) || empty($info)) {
            set_transient('epaync_update_subscription_error_msg', sprintf(__('An error has occurred during the cancellation or update of the subscription. Please consult the %s logs for more details.', 'woo-epaync-payment'), 'EpayNC'));
    ...
    private function get_order_details($order_id)
    {
        ...
        try {
            ...
            EpayncRestTools::checkResult($get_order_response);
    ...
    public function epaync_order_needs_payment($is_active, $order)
    {
        global $woocommerce;
    ...
    public function epaync_display_subscription_error_msg($column)
    {
        $epaync_cancelled_subscription_error_msg = get_transient('epaync_cancelled_subscription_error_msg');
        if ($epaync_cancelled_subscription_error_msg) {
            delete_transient('epaync_cancelled_subscription_error_msg');
            ?>
            <script type="text/javascript">
                jQuery(document).ready(function() {
                    if (! jQuery('#epaync_cancelled_subscription_error').length) {
                        jQuery('a.page-title-action').after('<div id="epaync_cancelled_subscription_error" class="error notice is-dismissible"><p><?php echo addslashes($epaync_cancelled_subscription_error_msg); ?></p><button type="button" class="notice-dismiss" onclick="this.parentElement.remove()"><span class="screen-reader-text"><?php echo esc_html__( 'Dismiss this notice.', 'woocommerce' )  ?></span></button></div>');
                    }
                });
            </script>
            <?php
    ...
    public function process_payment($order_id)
    {
        ...
        if (isset($_POST['epayncsubscription_old_pm'])) {
            set_transient($this->id . '_change_payment_' . $order_id, $_POST['epayncsubscription_old_pm']);
        }
