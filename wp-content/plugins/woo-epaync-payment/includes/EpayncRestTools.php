...
class EpayncRestTools
{
    public static function convertRestResult($answer)
    {
        ...
        $response['vads_currency'] = EpayncApi::getCurrencyNumCode(self::getProperty($transaction, 'currency'));
        ...
        if ($transactionDetails = self::getProperty($transaction, 'transactionDetails')) {
            ...
            $effectiveCurrency = EpayncApi::getCurrencyNumCode(self::getProperty($transactionDetails, 'effectiveCurrency'));
    ...
    public static function checkResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] != 'SUCCESS') {
            ...
            require_once 'EpayncRestException.php';
            throw new EpayncRestException($errorMessage, $answer['errorCode']);
        } elseif (! empty($expectedStatuses) && ! in_array($answer['detailedStatus'], $expectedStatuses)) {
            throw new Exception(sprintf(__('Unexpected transaction type received (%s).', 'woo-epaync-payment'), $answer['detailedStatus']));
        }
    }
}
