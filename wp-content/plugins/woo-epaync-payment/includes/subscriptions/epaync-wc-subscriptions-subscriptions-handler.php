...
class Epaync_WC_Subscriptions_Subscriptions_Handler implements Epaync_Subscriptions_Handler_Interface
{
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::init_hooks()
     */
    public function init_hooks()
    {
        add_action('woocommerce_subscription_pending-cancel_epayncsubscription', array($this, 'cancel_subscription'));
        add_action('wcs_subscription_schedule_after_billing_schedule', array($this, 'update_subscription'));
    }

    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::cart_contains_subscription()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::cart_contains_multiple_subscriptions()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::is_subscription_update()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::get_parent_order()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::subscription_info()
     */
    public function subscription_info($order)
    {
        $order_id = wcs_get_objects_property($order, 'id');

        $old_payment_method = get_transient('epayncsubscription_change_payment_' . $order_id);
        $is_payment_change = $old_payment_method ? true : false;
        delete_transient('epayncsubscription_change_payment_' . $order_id);

        // Payment method changes act on the subscription not the original order.
        if ($is_payment_change) {
            if ($old_payment_method === 'epayncsubscription') {
                return false;
            }
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::process_subscription()
     */
    public function process_subscription($order, $response)
    {
        ...
        update_post_meta($subscription->get_id(), 'Subscription amount', WC_Gateway_Epaync::display_amount($response->get('sub_amount'), $response->get('sub_currency')));
        update_post_meta($subscription->get_id(), 'Effect date', preg_replace('#^(\d{4})(\d{2})(\d{2})$#', '\1-\2-\3', $response->get('sub_effect_date')));

        if (WC_Gateway_Epaync::is_successful_action($response)) {
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::update_subscription()
     */
    public function process_subscription_renewal($order, $response)
    {
        ...
        if (wcs_get_objects_property($subscription, 'payment_method') !== 'epayncsubscription') {
            ...
            if (method_exists($subscription, 'set_payment_method')) {
                $subscription->set_payment_method($epaync_subscription);
            } else {
                $subscription->payment_method = $epaync_subscription->id;
                $subscription->payment_method_title = $epaync_subscription->get_title();
            }
        ...
        if ($renewal_order->has_status('pending')) {
            try {
                ...
                $order_note = sprintf(_x('IPN subscription payment for recurrence #%s.', 'used in order note', 'woo-epaync-payment'), $recurrence_number);
                $subscription->update_status('on-hold', $order_note);

                WC_Gateway_Epaync::epaync_add_order_note($response, $renewal_order);
                ...
                update_post_meta($renewal_order_id, 'Subscription amount', WC_Gateway_Epaync::display_amount($response->get('amount'), $currency_code));
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::cancel_subscription()
     */
    public function cancel_subscription()
    {
        ...
        $epaync_subscription = new WC_Gateway_EpayncSubscription();
        $epaync_subscription->cancel_online_subscription($subscription_id, $order_id);
    }
    ...
    public function update_subscription()
    {
        ...
        $epaync_subscription = new WC_Gateway_EpayncSubscription();
        $epaync_subscription->update_online_subscription($subscription_id, $order_id);
    }

    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::get_view_order_url()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Epaync_Subscriptions_Handler_Interface::get_subscription_statuses()
     */
    ...
    private function create_renewal_order($subscription)
    {
        ...
        if (is_wp_error($renewal_order)) {
            ...
            if (is_wp_error($renewal_order)) {
                throw new Exception(sprintf( __('Error: Unable to create renewal order with %s plugin.', 'woo-epaync-payment'), WC_Gateway_Epaync::GATEWAY_NAME));
            }
        }
