...
class WC_Gateway_EpayncChoozeo extends WC_Gateway_EpayncStd
{
    ...
    {
        $this->id = 'epayncchoozeo';
        $this->icon = apply_filters('woocommerce_epayncchoozeo_icon', WC_EPAYNC_PLUGIN_URL . 'assets/images/choozeo.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Choozeo payment', 'woo-epaync-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Choozeo payment.', 'woo-epaync-payment');

        $this->form_fields['title']['default'] = __('Payment with Choozeo without fees', 'woo-epaync-payment');
        ...
        $this->form_fields['multi_options'] = array(
            'title' => __('CHOOZEO PAYMENT OPTIONS', 'woo-epaync-payment'),
            'type' => 'title'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'description' => __('Define amount restriction for each card.', 'woo-epaync-payment')
        );
    }
        ...
        foreach ($options as $key => $option) {
            ...
            if (count($options) == 1) {
                echo '<input type="hidden"
                             id="epayncchoozeo_option_' . $key . '"
                             value="' . $key . '"
                             name="epayncchoozeo_option">';
            } else {
            ...
            echo '<label for="epayncchoozeo_option_' . $key . '" style="display: inline;">' . '
    ...
    public function process_payment($order_id)
    {
        ...
        $option = $_POST['epayncchoozeo_option'];
        $options = $this->get_available_options();
        $label = $options[$option]['label'];

        // Save selected payment option into session...
        set_transient('epayncchoozeo_option_' . $order_id, $option);
    ...
    protected function epaync_fill_request($order)
    {
        ...
        $option = get_transient('epayncchoozeo_option_' . self::get_order_property($order, 'id'));
        $this->epaync_request->set('payment_cards', $option);

        // By default WooCommerce does not manage customer type.
        $this->epaync_request->set('cust_status', 'PRIVATE');
        ...
        $this->epaync_request->set('cust_country', 'FR');

        delete_transient('epayncchoozeo_option_' . self::get_order_property($order, 'id'));
    }
}
