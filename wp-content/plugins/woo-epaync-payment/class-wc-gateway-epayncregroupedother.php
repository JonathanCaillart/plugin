...
class WC_Gateway_EpayncRegroupedOther extends WC_Gateway_EpayncStd
{
    public function __construct()
    {
        $this->id = 'epayncregroupedother';
        $this->icon = apply_filters('woocommerce_' . $this->id . '_icon', WC_EPAYNC_PLUGIN_URL . 'assets/images/other.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Other payment means', 'woo-epaync-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables this payment method.', 'woo-epaync-payment');

        $this->form_fields['title']['default'] = __('Other payment means', 'woo-epaync-payment');
        ...
        if (version_compare($woocommerce->version, '2.3.0', '>=')) {
            $this->form_fields['regroup_enabled'] = array(
                'title' => __('Regroup payment means', 'woo-epaync-payment'),
                'label' => __('Enable / disable', 'woo-epaync-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If this option is enabled, all the payment means added in this section will be displayed within the same payment submodule.', 'woo-epaync-payment')
            );
        }
        ...
        $descr = sprintf(__('Click on « Add » button to configure one or more payment means.<br /><b>Label: </b>The label of the means of payment to display on your site.<br /><b>Means of payment: </b>Choose the means of payment you want to propose.<br /><b>Countries: </b>Countries where the means of payment will be available. Leave blank to authorize all countries.<br /><b>Min. amount: </b>Minimum amount to enable the means of payment.<br /><b>Max. amount: </b>Maximum amount to enable the means of payment.<br /><b>Validation mode: </b>If manual is selected, you will have to confirm payments manually in your %s Back Office.<br /><b>Capture delay: </b>The number of days before the bank capture. Enter value only if different from %s general configuration.<br /><b>Cart data: </b>If you disable this option, the shopping cart details will not be sent to the gateway. Attention, in some cases, this option has to be enabled. For more information, refer to the module documentation.<br /><b>Do not forget to click on « Save » button to save your modifications.</b>',
            'woo-epaync-payment'), 'EpayNC', 'EpayNC');
        ...
        $columns['payment_mean'] = array(
            'title' => __('Means of payment', 'woo-epaync-payment'),
            'width' => '250px'
        );
        ...
        $columns['countries'] = array(
            'title' => __('Countries', 'woo-epaync-payment'),
            'width' => '175px',
        );
        ...
        $columns['send_cart_data'] = array(
            'title' => __('Cart data', 'woo-epaync-payment'),
            'width' => '92px',
        );

        $this->form_fields['payment_means'] = array(
            'title' => __('Payment means', 'woo-epaync-payment'),
            'type' => 'table',
            'columns' => $columns,
            'description' => $descr
        );
        ...
        $descr = $descr = sprintf(__('Click on « Add » button to add one or more new payment means.<br /><b>Code: </b>The code of the means of payment as expected by %s gateway.<br /><b>Label: </b>The default label of the means of payment.<br /><b>Do not forget to click on « Save » button to save your modifications.</b>',
            'woo-epaync-payment'), 'EpayNC');

        $culumns['code'] = array(
            'title' => __('Code', 'woo-epaync-payment'),
            'width' => '150px'
        );
        ...
        $this->form_fields['extra_payment_means'] = array(
            'title' => __('Add payment means', 'woo-epaync-payment'),
            'type' => 'table',
            'columns' => $culumns,
            'description' => $descr
        );
    ...
    {
        return __('Method title to display on payment means page. Used only if &laquo; Regroup payment means &raquo; option is enabled.', 'woo-epaync-payment');
    }

    protected function get_method_description_field_description()
    {
        return __('This controls the description which the user sees during checkout. Used only if &laquo; Regroup payment means &raquo; option is enabled.', 'woo-epaync-payment');
    }
    ...
    public function epaync_admin_head_script()
    {
        ...
        //<!--
                    ...
                    switch (attr) {
                        ...
                        case 'countries':
                            optionLine += '<div><select style="display:none; width: ' + width + 'px; height: 150px; background-color: white; padding: 2px;" name="' + inputName + '[]" id="' + inputName +
                                         '" multiple="multiple" onblur="javascript:epayncDisplayMultiSelect(\'' + inputName + '\'); epayncDisplayLabel(\'' + inputName + '\');">';
                            optionLine += '<?php
                                                ...
                                                if (labelValue == '') {
                                                    labelValue = '<?php echo __('Click to add countries.', 'woo-epaync-payment')?>';
                                                }

                            optionLine += '</select><label style="width:100%;" id="label_' + inputName + '" onclick="javascript:epayncDisplayMultiSelect(\'' + inputName + '\');" >' + labelValue + '</label></div>';
                            break;
                        ...
                        case 'send_cart_data':
                            optionLine += '<select style="width: ' + width + 'px;" name="' + inputName + '" id="' + inputName + '">';
                            optionLine += '<?php $options = array('n' => __('No', 'woo-epaync-payment'), 'y' => __('Yes', 'woo-epaync-payment'));
            ...
            function epayncDisplayLabel(selectId) {
                var select = document.getElementById(selectId);
                var label = document.getElementById('label_' + selectId);
            ...
            function getLabelText(select) {
                ...
                if (! labelText) {
                    labelText = '<?php echo __('Click to add countries.', 'woo-epaync-payment')?>';
                }
...
<?php
            ...
            if (($option['amount_min'] && (! is_numeric($option['amount_min']) || $option['amount_min'] < 0))
                ...
                if (in_array($option['payment_mean'], $used_cards)) {
                    ...
                    if (! $option['label']) {
                        ...
                        $value[$code]['label'] = sprintf(__('Payment with %s', 'woo-epaync-payment'), $cards[$option['payment_mean']]);
                    }
    ...
    public function validate_extra_payment_means_field($key, $value = null)
    {
        $name = $this->plugin_id . $this->id . '_' . $key;
        $value = $value ? $value : (key_exists($name, $_POST) ? $_POST[$name] : array());
        $used_cards = array_keys(EpayncApi::getSupportedCardTypes());
    ...
    protected function epaync_fill_request($order)
    {
        ...
        if (isset($option['capture_delay']) && is_numeric($option['capture_delay'])) {
            $this->epaync_request->set('capture_delay', $option['capture_delay']);
        }

        if (isset($option['validation_mode']) && ($option['validation_mode'] !== '-1')) {
            $this->epaync_request->set('validation_mode', $option['validation_mode']);
        }
