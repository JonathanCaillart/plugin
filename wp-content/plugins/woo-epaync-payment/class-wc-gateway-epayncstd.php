...
class WC_Gateway_EpayncStd extends WC_Gateway_Epaync
{
    const ALL_COUNTRIES = '1';
    const SPECIFIC_COUNTRIES = '2';

    protected $epaync_countries = array();
    protected $epaync_currencies = array();

    public function __construct()
    {
        $this->id = 'epayncstd';
        $this->icon = apply_filters('woocommerce_epayncstd_icon', WC_EPAYNC_PLUGIN_URL . '/assets/images/epaync.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Standard payment', 'woo-epaync-payment');
        ...
        add_action('woocommerce_api_wc_gateway_' . $this->id, array($this, 'epaync_generate_iframe_form'));

        // Return from REST payment action.
        add_action('woocommerce_api_wc_gateway_epaync_rest', array($this, 'epaync_rest_return_response'));

        // Notification from REST payment action.
        add_action('woocommerce_api_wc_gateway_epaync_notify_rest', array($this, 'epaync_rest_notify_response'));

        // Rest payment generate token.
        add_action('woocommerce_api_wc_gateway_epaync_form_token', array($this, 'epaync_refresh_form_token'));

        // Adding JS to load REST libs.
        add_action('wp_head', array($this, 'epaync_rest_head_script'));
    }

    public function epaync_rest_head_script()
    {
        if (in_array($this->get_option('card_data_mode'), array('REST', 'POPIN')) && $this->is_available()) {
            $epaync_pub_key = $this->testmode ? $this->get_general_option('test_public_key') : $this->get_general_option('prod_public_key');

            $locale = get_locale() ? substr(get_locale(), 0, 2) : null;
            if (! $locale || ! EpayncApi::isSupportedLanguage($locale)) {
                $locale = $this->settings['language'];
            }

            $language_iso_code = $locale;
            $return_url = add_query_arg('wc-api', 'WC_Gateway_Epaync_Rest', network_home_url('/'));
            $custom_placeholders = '';
            ...
            // Custom theme.
            $epaync_std_rest_theme = $this->settings['rest_theme'];
            $epaync_static_url = $this->get_general_option('static_url', self::STATIC_URL);

            ?>
                <script>
                    var EPAYNC_LANGUAGE = "<?php echo $language_iso_code; ?>"
                </script>
                <script src="<?php echo $epaync_static_url; ?>js/krypton-client/V4.0/stable/kr-payment-form.min.js"
                        kr-public-key="<?php echo $epaync_pub_key; ?>"
                ...
                <link rel="stylesheet" href="<?php echo $epaync_static_url; ?>js/krypton-client/V4.0/ext/<?php echo $epaync_std_rest_theme;?>-reset.css">
                <script src="<?php echo $epaync_static_url; ?>js/krypton-client/V4.0/ext/<?php echo $epaync_std_rest_theme;?>.js"></script>

                <style>
                    #epayncstd_rest_wrapper button.kr-popin-button {
            ...
            // Load REST script.
            wp_register_script('rest-js', WC_EPAYNC_PLUGIN_URL . 'assets/js/rest.js');
    ...
    {
        if (empty($this->epaync_countries)) {
            return $countries;
        } else {
            $allowed_countries = array();
            foreach ($this->epaync_countries as $code) {
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'module_settings' => array(
                'title' => __('MODULE SETTINGS', 'woo-epaync-payment'),
                'type' => 'title'
            ),
            'enabled' => array(
                'title' => __('Activation', 'woo-epaync-payment'),
                'label' => __('Enable / disable', 'woo-epaync-payment'),
                'type' => 'checkbox',
                'default' => 'yes',
                'description' => __('Enables / disables standard payment.', 'woo-epaync-payment')
            ),
            'title' => array(
                'title' => __('Title', 'woo-epaync-payment'),
                'type' => 'text',
                'description' => $this->get_method_title_field_description(),
                'default' => __('Payment by credit card', 'woo-epaync-payment')
            ),
            'description' => array(
                'title' => __('Description', 'woo-epaync-payment'),
                'type' => 'textarea',
                'description' => $this->get_method_description_field_description(),
                'default' => __('You will enter payment data after order confirmation.', 'woo-epaync-payment'),
                'css' => 'width: 35em;'
            ),

            // Amount restrictions.
            'restrictions' => array(
                'title' => __('RESTRICTIONS', 'woo-epaync-payment'),
                'type' => 'title'
            ),
            'allows_specific' => array(
                'custom_attributes' => array(
                    'onchange' => 'epayncUpdateSpecificCountriesDisplay()'
                ),
                'title' => __('Restrict to some countries', 'woo-epaync-payment'),
                'type' => 'select',
                'default' => '1',
                'options' => array(
                    self::ALL_COUNTRIES => __('All allowed countries', 'woo-epaync-payment'),
                    self::SPECIFIC_COUNTRIES => __('Specific countries', 'woo-epaync-payment')
                ),
                'class' => 'wc-enhanced-select',
                'description' => __('Buyer\'s billing countries in which this payment method is available.', 'woo-epaync-payment')
            ),
            'specific_countries' => array(
                'title' => __('Authorized countries', 'woo-epaync-payment'),
                'type' => 'multiselect',
                'default' => '',
            ...
            'amount_min' => array(
                'title' => __('Minimum amount', 'woo-epaync-payment'),
                'type' => 'text',
                'default' => '',
                'description' => __('Minimum amount to activate this payment method.', 'woo-epaync-payment')
            ),
            'amount_max' => array(
                'title' => __('Maximum amount', 'woo-epaync-payment'),
                'type' => 'text',
                'default' => '',
                'description' => __('Maximum amount to activate this payment method.', 'woo-epaync-payment')
            ),
            ...
            'capture_delay' => array(
                ...
                'type' => 'text',
                'default' => '',
                'description' => sprintf(__('The number of days before the bank capture. Enter value only if different from %s general configuration.', 'woo-epaync-payment'), self::GATEWAY_NAME)
            ),
            ...
            'payment_cards' => array(
                'title' => __('Card Types', 'woo-epaync-payment'),
                'type' => 'multiselect',
                'default' => array(),
                'options' => $this->get_supported_card_types(),
                'description' => __('The card type(s) that can be used for the payment. Select none to use gateway configuration.', 'woo-epaync-payment'),
                'class' => 'wc-enhanced-select'
            ),

            // Advanced options.
            'advanced_options' => array(
                'title' => __('ADVANCED OPTIONS', 'woo-epaync-payment'),
                'type' => 'title'
            ),
            'card_data_mode' => array(
                'custom_attributes' => array(
                    'onchange' => 'epayncUpdateRestFieldDisplay(false)'
                ),
                'title' => __('Card data entry mode', 'woo-epaync-payment'),
                'type' => 'select',
                'default' => 'DEFAULT',
                'options' => array(
                    'DEFAULT' => __('Card data entry on payment gateway', 'woo-epaync-payment'),
                    'MERCHANT' => __('Card type selection on merchant site', 'woo-epaync-payment'),
                    'IFRAME' => __('Payment page integrated to checkout process (iframe)', 'woo-epaync-payment'),
                ),
                'description' => __('Select how the credit card data will be entered by buyer. Think to update payment method description to match your selected mode.', 'woo-epaync-payment'),
                'class' => 'wc-enhanced-select'
            )
        );

        // Add REST fields if available for payment.
        if ($epaync_plugin_features['embedded']) {
            $this->form_fields['card_data_mode']['options']['REST'] = __('Embedded payment fields on merchant site (REST API)', 'woo-epaync-payment');
            $this->form_fields['card_data_mode']['options']['POPIN'] = __('Embedded payment fields in a pop-in (REST API)', 'woo-epaync-payment');
        ...
        $this->form_fields['payment_by_token'] = array(
            'custom_attributes' => array(
                'onchange' => 'epayncUpdatePaymentByTokenField()',
            ),
            'title' => __('Payment by token', 'woo-epaync-payment'),
            'type' => 'select',
            'default' => '0',
            'options' => array(
                '1' => __('Yes', 'woo-epaync-payment'),
                '0' => __('No', 'woo-epaync-payment')
            ),
            'description' => sprintf(__('The payment by token allows to pay orders without re-entering bank data at each payment. The "Payment by token" option should be enabled on your %s store to use this feature.', 'woo-epaync-payment'), self::GATEWAY_NAME),
            'class' => 'wc-enhanced-select'
        );
    ...
    protected function get_rest_fields()
    {
        ...
        $this->form_fields['rest_customization'] = array(
            'title' => __('CUSTOMIZATION', 'woo-epaync-payment'),
            'type' => 'title',
        );

        $this->form_fields['rest_theme'] = array(
            'title' => __('Theme', 'woo-epaync-payment'),
            'type' => 'select',
            'default' => 'material',
            'options' => array(
                'classic' => 'Classic',
            ...
            'description' => __('Select a theme to use to display embedded payment fields. For more customization, you may edit module template manually.', 'woo-epaync-payment'),
            'class' => 'wc-enhanced-select'
        );

        $this->form_fields['rest_placeholder'] = array(
            'title' => __('Custom fields placeholders', 'woo-epaync-payment'),
            ...
            'description' => __('Texts to use as placeholders for embedded payment fields.', 'woo-epaync-payment')
        );

        $this->form_fields['rest_register_card_label'] = array(
            'title' => __('Register card label', 'woo-epaync-payment'),
            'type' => 'text',
            'default' => __('Register my card', 'woo-epaync-payment'),
            'description' => __('Label displayed to invite buyers to register their card data.', 'woo-epaync-payment')
        );

        $this->form_fields['rest_attempts'] = array(
            'title' => __('Payment attempts number', 'woo-epaync-payment'),
            'type' => 'text',
            'description' => __('Maximum number of payment retries after a failed payment (between 0 and 9). If blank, the gateway default value is 3.', 'woo-epaync-payment')
        );
    ...
    public function generate_placeholder_table_html($key, $data)
    {
        ...
        $html .= '<tr>
                   <td>
                     <label style="color: #23282d; font-weight: 500;" for="' . $field_name . '_pan">' . __('Card number', 'woo-epaync-payment') . '</label>
                   </td>
                   <td>
        ...
        $html .= '<tr>
                   <td>
                     <label style="color: #23282d; font-weight: 500;" for="' . $field_name . '_expiry">' . __('Expiry date', 'woo-epaync-payment') . '</label>
                   </td>
                   <td>
        ...
        $html .= '<tr>
                   <td>
                     <label style="color: #23282d; font-weight: 500;" for="' . $field_name . '_cvv">' . __('CVV', 'woo-epaync-payment') . '</label>
                   </td>
                   <td>
    ...
    public function epaync_admin_head_script()
    {
        ...
        //<!--
            jQuery(function() {
                epayncUpdateSpecificCountriesDisplay();
            });

            function epayncUpdateSpecificCountriesDisplay() {
            ...
            jQuery(document).ready(function() {
                epayncUpdateRestFieldDisplay();
            });

            function epayncUpdateRestFieldDisplay(ignoreIframe = true) {
                ...
                if (jQuery.inArray(cardDataMode, ['REST', 'POPIN']) != -1) {
                    ...
                    if (! ignoreIframe) {
                        if ((cardDataMode === 'IFRAME') &&
                            ! confirm('<?php echo __('Warning, some payment means are not compatible with an integration by iframe. Please consult the documentation for more details.', 'woo-epaync-payment')?>')) {
    ...
    {
        if (! empty($this->epaync_currencies)) {
            return in_array(get_woocommerce_currency(), $this->epaync_currencies);
        }
    ...
    protected function is_available_for_country()
    {
        ...
        if ($this->get_option('allows_specific') === self::ALL_COUNTRIES) {
            return empty($this->epaync_countries) || in_array($country, $this->epaync_countries);
        }
    ...
    protected function is_available_for_subscriptions()
    {
        ...
        if (class_exists('WC_Gateway_EpayncSubscription')) {
            ...
            $handler = is_array($settings) && isset($settings['subscriptions']) ? $settings['subscriptions'] :
                WC_Gateway_EpayncSubscription::SUBSCRIPTIONS_HANDLER;
            $subscriptions_handler = Epaync_Subscriptions_Loader::getInstance($handler);
    ...
    public function payment_fields()
    {
        ...
        switch ($this->get_option('card_data_mode')) {
            ...
            case 'IFRAME':
                ...
                $html .= '<div>
                         <iframe name="epaync_iframe" id="epaync_iframe" src="' . add_query_arg('loading', 'true', $link) . '" style="display: none;">
                         </iframe>';
                ...
                $html .= "\njQuery('form.checkout').on('checkout_place_order_" . $this->id . "', function() {
                                ...
                                jQuery.ajaxPrefilter(function(options, originalOptions, jqXHR) {
                                    ...
                                    options.success = function(data, status, jqXHR) {
                                        ...
                                        jQuery('#epaync_iframe').show();

                                        jQuery('#epaync_iframe').attr('src', '$link');
                ...
                $html .= "\njQuery('input[type=\"radio\"][name=\"payment_method\"][value!=\"" . $this->id . "\"]').click(function() {
                                ...
                                jQuery('#epaync_iframe').hide();

                                jQuery('#epaync_iframe').attr('src', '" . add_query_arg('loading', 'true', $link) . "');
                            });";
                $html .= "\n</script>";
                $html .= "\n</div>";
                break;
            ...
            case 'POPIN':
                ...
                $form_token_url = add_query_arg('wc-api', 'WC_Gateway_Epaync_Form_Token', home_url('/'));

                $html .= "\n" . '<script type="text/javascript">';
                $html .= "\n  var savedData = false;";

                $html .= "\n  jQuery('form.checkout').on('checkout_place_order_epayncstd', function() {
                                jQuery('form.checkout').removeClass('processing').unblock();
                                ...
                                jQuery.ajaxPrefilter(function(options, originalOptions, jqXHR) {
                                    ...
                                    if (options.data.indexOf('payment_method=epayncstd') == -1) {
                                        return;
                                    }
                                    ...
                                    options.success = function(data, status, jqXHR) {
                                        ...
                                        if (! popin) {
                                            jQuery('#epayncstd_rest_processing').css('display', 'block');
                                            jQuery('ul." . $this->id . "-view-top li.block').hide();
                                            jQuery('ul.epayncstd-view-bottom').hide();
                                        }
                                        ...
                                        if (savedData && (newData === savedData)) {
                                            ...
                                            var useIdentifier = jQuery('#epaync_use_identifier').length && jQuery('#epaync_use_identifier').val();
                                            savedData = newData;
                                            jQuery.ajax({
                                                method: 'POST',
                                                url: '" . $form_token_url . "',
        ...
        if ($can_pay_by_alias) {
            ...
            if ($force_redir) {
                ...
                echo '<input type="hidden" name="epaync_force_redir" value="true">';
            } else {
                echo '<div>';
    ...
    protected function payment_by_alias_view($payment_fields, $force_redir)
    {
        ...
        echo '    </li>
                  ...
                  <li class="block ' . $this->id . '-id-block">
                      <input id="epaync_use_identifier" type="hidden" value="true" name="epaync_use_identifier">
                      <span>' .
                          sprintf(__('You will pay with your stored means of payment %s', 'woo-epaync-payment'), $saved_masked_pan)
                          . ' (<a href="' . esc_url(wc_get_account_endpoint_url($this->get_option('woocommerce_saved_cards_endpoint', 'ly_saved_cards'))) . '">' . __('manage your payment means', 'woo-epaync-payment') . '</a>).
                      </span>
                  </li>';
        ...
        echo '</ul>
              ...
              <ul class="epayncstd-view-bottom" style="margin-left: 0; margin-top: 0;">
                  <li style="margin: 15px 0px;" class="block ' . $this->id . '-cc-block ' . $this->id . '-id-block">
                      <span>' . __('OR', 'woo-epaync-payment') . '</span>
                  </li>

                  <li class="block ' . $this->id . '-cc-block">
                      <a href="javascript: void(0);" onclick="epayncUpdatePaymentBlock(true)">' . __('Click here to pay with your registered means of payment.', 'woo-epaync-payment') . '</a>
                  </li>

                  <li class="block ' . $this->id . '-id-block">
                      <a href="javascript: void(0);" onclick="epayncUpdatePaymentBlock(false)">' . __('Click here to pay with another means of payment.', 'woo-epaync-payment') . '</a>
                  </li>
              </ul>';

        echo '<script type="text/javascript">
                  function epayncUpdatePaymentBlock(useIdentifier) {
                      jQuery("ul.' . $this->id . '-view-top li.block").hide();
                      jQuery("ul.epayncstd-view-bottom li.block").hide();

                      var blockName = useIdentifier ? "id" : "cc";
                      jQuery("li.' . $this->id . '-" + blockName + "-block").show();

                      if (typeof epayncUpdateFormToken === "function") {
                          epayncUpdateFormToken(useIdentifier);
                      }

                      jQuery("#epaync_use_identifier").val(useIdentifier);
                  }

                  epayncUpdatePaymentBlock(true);
    ...
    private function rest_payment_fields_view($use_identifier)
    {
        ...
        $img_url = WC_EPAYNC_PLUGIN_URL . 'assets/images/loading.gif';
        ...
        $html .= '<div id="epayncstd_rest_wrapper"></div>';
        ...
        $html .= "\n" . '
                    var epayncDrawRestPaymentFields = function(formToken, first) {
                        var fields = \'<div class="kr-embedded" '. $popin_attr . '>\' +
                                     ...
                                     \'    <div id="epayncstd_rest_processing" class="kr-field processing" style="display: none; border: none;">\' +
                        ...
                        jQuery("#epayncstd_rest_wrapper").html(fields);
                        ...
                        setTimeout(function () {
                            ...
                            KR.setFormConfig({
                                ...
                                if (first) {
                                    epayncInitRestEvents(v.KR);
                                }
                            });
                        }, 300);
                    };

                    var epayncUpdateFormToken = function(useIdentifier) {
                        ...
                        epayncDrawRestPaymentFields(formToken, ! KR || ! KR.vueReady);
                    };

                    var useIdentifier = typeof IDENTIFIER_FORM_TOKEN !== "undefined";
                    if (! useIdentifier) {
                        setTimeout(function () {
                            epayncUpdateFormToken(false);
                        }, 300);
                    }
                    ...
                    jQuery (document).ready(function(){
                        jQuery("#place_order").click(function(event) {
                            if (! jQuery("#payment_method_epayncstd").is(":checked")) {
                                return true;
                            }

                            var useIdentifier = jQuery("#epaync_use_identifier").length && jQuery("#epaync_use_identifier").val() === "true";
                            var popin = jQuery(".kr-popin-button").length > 0;
    ...
    private function get_temporary_form_token($use_identifier = false)
    {
        ...
        try {
            $client = new EpayncRest($this->get_general_option('rest_url'), $this->get_general_option('site_id'), $key);
    ...
    private function get_form_token($order, $use_identifier = false)
    {
        ...
        $order_id = $this->get_escaped_var($this->epaync_request, 'order_id');
        $currency = EpayncApi::findCurrencyByAlphaCode(get_woocommerce_currency());
        ...
        $params = array(
            'orderId' => $order_id,
            'customer' => array(
                'email' => $this->get_escaped_var($this->epaync_request, 'cust_email'),
                'reference' => $this->get_escaped_var($this->epaync_request, 'cust_id'),
                'billingDetails' => array(
                    'language' => $this->get_escaped_var($this->epaync_request, 'language'),
                    'title' => $this->get_escaped_var($this->epaync_request, 'cust_title'),
                    'firstName' => $this->get_escaped_var($this->epaync_request, 'cust_first_name'),
                    'lastName' => $this->get_escaped_var($this->epaync_request, 'cust_last_name'),
                    'category' => $this->get_escaped_var($this->epaync_request, 'cust_status'),
                    'address' => $this->get_escaped_var($this->epaync_request, 'cust_address'),
                    'zipCode' => $this->get_escaped_var($this->epaync_request, 'cust_zip'),
                    'city' => $this->get_escaped_var($this->epaync_request, 'cust_city'),
                    'state' => $this->get_escaped_var($this->epaync_request, 'cust_state'),
                    'phoneNumber' => $this->get_escaped_var($this->epaync_request, 'cust_phone'),
                    'country' => $this->get_escaped_var($this->epaync_request, 'cust_country')
                ),
                'shippingDetails' => array(
                    'firstName' => $this->get_escaped_var($this->epaync_request, 'ship_to_first_name'),
                    'lastName' => $this->get_escaped_var($this->epaync_request, 'ship_to_last_name'),
                    'category' => $this->get_escaped_var($this->epaync_request, 'ship_to_status'),
                    'address' => $this->get_escaped_var($this->epaync_request, 'ship_to_street'),
                    'address2' => $this->get_escaped_var($this->epaync_request, 'ship_to_street2'),
                    'zipCode' => $this->get_escaped_var($this->epaync_request, 'ship_to_zip'),
                    'city' => $this->get_escaped_var($this->epaync_request, 'ship_to_city'),
                    'state' => $this->get_escaped_var($this->epaync_request, 'ship_to_state'),
                    'phoneNumber' => $this->get_escaped_var($this->epaync_request, 'ship_to_phone_num'),
                    'country' => $this->get_escaped_var($this->epaync_request, 'ship_to_country'),
                    'deliveryCompanyName' => $this->get_escaped_var($this->epaync_request, 'ship_to_delivery_company_name'),
                    'shippingMethod' => $this->get_escaped_var($this->epaync_request, 'ship_to_type'),
                    'shippingSpeed' => $this->get_escaped_var($this->epaync_request, 'ship_to_speed')
                )
            ),
            'transactionOptions' => array(
                'cardOptions' => array('captureDelay' => $this->get_escaped_var($this->epaync_request, 'capture_delay'),
                    'manualValidation' => ($this->get_escaped_var($this->epaync_request, 'validation_mode') == '1') ? 'YES' : 'NO',
                    'paymentSource' => 'EC'
                )
            ),
            'contrib' => $this->get_escaped_var($this->epaync_request, 'contrib'),
            'strongAuthentication' => $strong_auth,
            'currency' => $currency->getAlpha3(),
            'amount' => $this->get_escaped_var($this->epaync_request, 'amount'),
            'metadata' => array(
        ...
        if ($use_identifier) {
            ...
            $this->log('Customer ' . $this->epaync_request->get('cust_email') . ' will be asked for card data registration.');
            $params['formAction'] = 'ASK_REGISTER_PAY';
        }
        ...
        try {
            $client = new EpayncRest($this->get_general_option('rest_url'), $this->epaync_request->get('site_id'), $key);
            $result = $client->post('V4/Charge/CreatePayment', json_encode($params));
    ...
    public function epaync_refresh_form_token()
    {
        global $woocommerce;
        ...
        $this->epaync_fill_request($order);
    ...
    public function process_payment($order_id)
    {
        ...
        // Set flag about use of saved identifier.
        if (isset($_POST['epaync_use_identifier'])) {
            set_transient($this->id . '_use_identifier_' . $order_id, $_POST['epaync_use_identifier'] === 'true');
        }

        $order = new WC_Order($order_id);

        // If $_POST['epaync_force_redir'] is set, force payment by redirection.
        if (in_array($this->get_option('card_data_mode'), array('REST', 'POPIN')) && ! isset($_POST['epaync_force_redir'])) {
            return array(
                'result' => 'success'
            );
        }
     ...
     **/
    public function epaync_generate_form($order_id)
    {
        global $woocommerce;
        ...
        echo __('Please wait, you will be redirected to the payment gateway.', 'woo-epaync-payment');
        echo '</div>';
        echo '<br />';
        echo '<p>' . __('If nothing happens in 10 seconds, please click the button below.', 'woo-epaync-payment') . '</p>';

        $this->epaync_fill_request($order);

        // Log data that will be sent to payment gateway.
        $this->log('Data to be sent to payment gateway : ' . print_r($this->epaync_request->getRequestFieldsArray(true /* To hide sensitive data. */), true));

        $form = "\n".'<form action="' . esc_url($this->epaync_request->get('platform_url')) . '" method="post" id="' . $this->id . '_payment_form">';
        $form .= "\n" . $this->epaync_request->getRequestHtmlFields();
        $form .= "\n" . '  <input type="submit" class="button-alt" id="' . $this->id . '_payment_form_submit" value="' . sprintf(__('Pay via %s', 'woo-epaync-payment'), self::GATEWAY_NAME).'">';
        $form .= "\n" . '  <a class="button cancel" href="' . esc_url($order->get_cancel_order_url()) . '">' . __('Cancel order &amp; restore cart', 'woo-epaync-payment') . '</a>';
        $form .= "\n" . '</form>';

        $form .= "\n".'<script type="text/javascript">';
        $form .= "\nfunction epaync_submit_form() {
                    document.getElementById('" . $this->id . "_payment_form_submit').click();
                  }";
        $form .= "\nif (window.addEventListener) { // For all major browsers.
                    window.addEventListener('load', epaync_submit_form, false);
                  } else if (window.attachEvent) { // For IE 8 and earlier versions.
                    window.attachEvent('onload', epaync_submit_form);
                  }";
        $form .= "\n</script>\n";
    ...
    public function epaync_generate_iframe_form()
    {
        global $woocommerce;
        ...
        $this->epaync_request->set('theme_config', '3DS_LOGOS=false;');

        $this->epaync_request->set('action_mode', 'IFRAME');
        $this->epaync_request->set('redirect_enabled', '1');
        $this->epaync_request->set('redirect_success_timeout', '0');
        $this->epaync_request->set('redirect_error_timeout', '0');

        // Log data that will be sent to payment gateway.
        $this->log('Data to be sent to payment gateway : ' . print_r($this->epaync_request->getRequestFieldsArray(true /* To hide sensitive data. */), true));

        $form = "\n" . '<form action="' . esc_url($this->epaync_request->get('platform_url')) . '" method="post" id="' . $this->id . '_payment_iframe_form">';
        $form .= "\n" . $this->epaync_request->getRequestHtmlFields();
        $form .= "\n" . '</form>';
    ...
    protected function epaync_fill_request($order)
    {
        ...
        if ($currency == null) {
            ...
            wp_die(sprintf(__('The store currency (%s) is not supported by %s.', 'woo-epaync-payment'), get_woocommerce_currency(), self::GATEWAY_NAME));
        }

        // Params.
        $misc_params = array(
            'amount' => $currency->convertAmountToInteger($order->get_total()),
            'contrib' => EpayncTools::get_contrib(),
            ...
            // Return URLs.
            'url_return' => add_query_arg('wc-api', 'WC_Gateway_Epaync', home_url('/'))
        );
        $this->epaync_request->setFromArray($misc_params);

        $this->epaync_request->addExtInfo('order_key', self::get_order_property($order, 'order_key'));
        $this->epaync_request->addExtInfo('blog_id', $wpdb->blogid);

        // VAT amount for colombian payment means.
        $this->epaync_request->set('totalamount_vat', $currency->convertAmountToInteger($order->get_total_tax()));
        ...
        $this->epaync_request->set('threeds_mpi', $threeds_mpi);

        // Detect language.
        $locale = get_locale() ? substr(get_locale(), 0, 2) : null;
        if ($locale && EpayncApi::isSupportedLanguage($locale)) {
            $this->epaync_request->set('language', $locale);
        } else {
            $this->epaync_request->set('language', $this->get_general_option('language'));
        }

        // Available languages.
        $langs = $this->get_general_option('available_languages');
        if (is_array($langs) && ! in_array('', $langs)) {
            $this->epaync_request->set('available_languages', implode(';', $langs));
        }
        ...
        if (isset($this->form_fields['card_data_mode'])) {
            ...
            if ($this->get_option('card_data_mode') === 'MERCHANT') {
                ...
                $this->epaync_request->set('payment_cards', $selected_card);

                delete_transient($this->id . '_card_type_' . $order_id);
            } else {
                $cards = $this->get_option('payment_cards');
                if (is_array($cards) && ! in_array('', $cards)) {
                    $this->epaync_request->set('payment_cards', implode(';', $cards));
                }
            }
        }

        // Enable automatic redirection?
        $this->epaync_request->set('redirect_enabled', ($this->get_general_option('redirect_enabled') == 'yes') ? true : false);
        ...
        $this->epaync_request->set('redirect_success_message', $success_message);
        ...
        $this->epaync_request->set('redirect_error_message', $error_message);
        ...
        foreach ($config_keys as $key) {
            $this->epaync_request->set($key, $this->get_general_option($key));
        }

        // Check if capture_delay and validation_mode are overriden in submodules.
        if (is_numeric($this->get_option('capture_delay'))) {
            $this->epaync_request->set('capture_delay', $this->get_option('capture_delay'));
        }

        if ($this->get_option('validation_mode') !== '-1') {
            $this->epaync_request->set('validation_mode', $this->get_option('validation_mode'));
        }
    ...
    protected function send_cart_data($order)
    {
        ...
        foreach ($order->get_items() as $line_item) {
            ...
            $this->epaync_request->addProduct(
                $item_data['name'],
     ...
     **/
    public function epaync_rest_return_response()
    {
        $this->epaync_manage_rest_notify_response(false);
    }

    /**
     * Check for REST notification response.
     **/
    public function epaync_rest_notify_response()
    {
        $this->epaync_manage_rest_notify_response(true);
    }

    public function epaync_manage_rest_notify_response($from_server_rest = false)
    {
        global $woocommerce ;
        ...
        if (! EpayncRestTools::checkResponse($raw_response)) {
        ...
        if (! EpayncRestTools::checkHash($raw_response, $sha_key)) {
        ...
        $data = EpayncRestTools::convertRestResult($answer);
        $response = new EpayncResponse($data, null, null, null);

        parent::epaync_manage_notify_response($response, $from_server_rest);
    }
