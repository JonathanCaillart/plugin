...
class WC_Gateway_EpayncFranfinance extends WC_Gateway_EpayncStd
{

    protected $epaync_countries = array('FR', 'GP', 'MQ', 'GF', 'RE', 'YT'); // France and DOM.
    protected $epaync_currencies = array('EUR');

    public function __construct()
    {
        $this->id = 'epayncfranfinance';
        $this->icon = apply_filters('woocommerce_epayncfranfinance_icon', WC_EPAYNC_PLUGIN_URL . 'assets/images/franfinance.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Franfinance payment', 'woo-epaync-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Franfinance payment.', 'woo-epaync-payment');

        $this->form_fields['title']['default'] = __('Payment with Franfinance', 'woo-epaync-payment');
        ...
        $this->form_fields['payment_options'] = array(
            'title' => __('FRANFINANCE PAYMENT OPTIONS', 'woo-epaync-payment'),
            'type' => 'title'
        );
        ...
        $columns['fees'] = array(
            'title' => __('Fees', 'woo-epaync-payment'),
            'default' => '-1',
            'width' => '125px'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'columns' => $columns,
            'default' => array (
                '3x' => array('label' => sprintf(__('Payment in %s times', 'woo-epaync-payment'), '3'), 'count' => '3', 'fees' => '-1', 'amount_min' => '100', 'amount_max' => '3000'),
                '4x' => array('label' => sprintf(__('Payment in %s times', 'woo-epaync-payment'), '4'), 'count' => '4', 'fees' => '-1', 'amount_min' => '100', 'amount_max' => '4000'),
            ),
            ...
            function epayncAddOption(fieldName, record, key) {
                ...
                jQuery.each(orderedRecord, function(attr, value) {
                    ...
                    switch (attr) {
                        ...
                        case 'fees':
                            optionLine += '<select style="width: ' + width + 'px;" name="' + inputName + '" id="' + inputName + '">';
                            optionLine += '<?php $options = array('-1' => sprintf(__('%s Back Office configuration', 'woo-epaync-payment'), self::BACKOFFICE_NAME), 'N' => __('Without fees', 'woo-epaync-payment'), 'Y' => __('With fees', 'woo-epaync-payment'));
                                                foreach ($options as $key => $value) {
                                                    echo '<option value="' . $key . '">' . $value . '</option>';
                                                } ?>';
...
<?php
    ...
    public function validate_payment_options_field($key, $value = null)
    {
        ...
        foreach ($value as $code => $option) {
            ...
            if (! $option['label']) {
                $value[$code]['label'] = sprintf(__('Payment in %s times', 'woo-epaync-payment'), $option['count']);
            }
        }
    ...
    public function payment_fields()
    {
        ...
        if (count($options) == 1) {
            ...
            echo '<li style="list-style-type: none;">
                    <input type="hidden" id="epayncfranfinance_option_' . $key . '" value="' . $key . '" name="epayncfranfinance_option">
            ...
            foreach ($options as $key => $option) {
                echo '<li style="list-style-type: none;">
                        <input class="radio" type="radio"'. ($first == true ? ' checked="checked"' : '') . ' id="epayncfranfinance_option_' . $key . '" value="' . $key . '" name="epayncfranfinance_option">
                        <label for="epayncfranfinance_option_' . $key . '" style="display: inline;">' . $option['label'] . '</label>
                      </li>';
                $first = false;
            }
        }
    ...
    public function process_payment($order_id)
    {
        ...
        $options = $this->get_available_options();
        $option = $options[$_POST['epayncfranfinance_option']];

        // Save selected payment option into session...
        set_transient('epayncfranfinance_option_' . $order_id, $option);
    ...
    protected function epaync_fill_request($order)
    {
        ...
        $this->epaync_request->set('validation_mode', '0');
        $this->epaync_request->set('capture_delay', '0');

        $option = get_transient('epayncfranfinance_option_' . self::get_order_property($order, 'id'));
        $this->epaync_request->set('payment_cards', 'FRANFINANCE_' . $option['count'] . 'X');

        if ($option['fees'] !== '-1') {
            $fees = $option['fees'] ? 'Y' : 'N';
            $this->epaync_request->set('acquirer_transient_data', '{"FRANFINANCE":{"FEES_' . $option['count'] . 'X":"' . $fees . '"}}');
        }

        delete_transient('epayncfranfinance_option_' . self::get_order_property($order, 'id'));
    }
}
