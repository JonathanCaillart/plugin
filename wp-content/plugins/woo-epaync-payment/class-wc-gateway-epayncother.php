...
class WC_Gateway_EpayncOther extends WC_Gateway_EpayncStd
{
    ...
    public function __construct($payment_code, $payment_title)
    {
        ...
        $this->regrouped_other_payments = new WC_Gateway_EpayncRegroupedOther();

        $code = strtolower($this->payment_code);
        $this->id = 'epayncother_' . $code;
    ...
    public function get_title()
    {
        ...
        if (! $title) {
            ...
            $title = sprintf(__('Payment with %s', 'woo-epaync-payment'), $cards[$this->payment_code]);
        }
    ...
    protected function epaync_fill_request($order)
    {
        ...
        // Set payment card.
        $this->epaync_request->set('payment_cards', $this->payment_code);
